...
public class TreeDistancesI {
	...
	static int [] d1 = new int[200001];  // to store first-max length.
	static int [] d2 = new int[200001]; // to store second-max length.
	static int [] c = new int[200001]; // to store child for path of max length.
...
[here](https://usaco.guide/CPH.pdf#page=146). Once we have a diameter $(a,b)$,
